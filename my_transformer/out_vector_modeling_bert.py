...
class MyBertSelfAttention(nn.Module):
	...
	def forward(
		...
		# (batch size, head num, text_a_seq_len + candidate_num, dim)
...
class MyBertAttention(nn.Module):
	...
	def forward(
		...
		outputs = (attention_output, candidate_embeddings) + self_outputs[3:]  # add attentions if we output them
...
class MyBertEncoder(nn.Module):
	...
	def forward(
		...
		for i, layer_module in enumerate(self.layer):
			...
			# add by me
			this_layer_enrich_candidate_by_question = enrich_candidate_by_question[i]
			...
			if self.gradient_checkpointing and self.training:
				...
				layer_outputs = torch.utils.checkpoint.checkpoint(
					...
					encoder_hidden_states,
					encoder_attention_mask,
					enrich_candidate_by_question=this_layer_enrich_candidate_by_question,
			...
			if this_layer_enrich_candidate_by_question:
